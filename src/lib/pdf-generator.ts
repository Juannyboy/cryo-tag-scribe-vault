...
export const generateQROnlyPDF = (record: DecanterRecord) => {
  ...
  // Get QR code canvas - specifically look for the record-qr ID
  const qrCanvas = document.getElementById(`record-qr-${record.id}`) as HTMLCanvasElement;
  ...
  if (qrCanvas) {
    ...
    const recordUrl = `${baseUrl}/record/${record.id}`;
    doc.setFontSize(8);
    doc.text(`Scan or visit: ${recordUrl}`, pageWidth / 2, y + qrSize + 20, { align: 'center' });
